...
from gridt_server.models import User, Movement, Signal, MovementUserAssociation
...
class MovementTest(BaseTest):
    ...
    def test_dictify(self):
        with self.app_context():
            ...
            with freeze_time(later, tz_offset=1):
                self.signal_as_user(self.users[1], self.movements[0], message=message)
            ...
            expected = {
                ...
                "subscribed": True,
                "interval": "daily",
                "last_signal_sent": {"time_stamp": later},
                "leaders": [user_dict],
            }
    ...
    @patch(
        "gridt_server.models.signal.Signal._get_now", return_value=datetime(1996, 3, 15)
    )
